...
<div class="py-6">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20-md">
            <form action="{{ route('admin.events.update', $event) }}" method="POST" id="editEventForm" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="4"
                                      class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-rose-500 focus:border-transparent @error('description') border-red-500 @enderror">{{ old('description', $event->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            @if($event->registrations()->count() > 0)
                                <p class="mt-1 text-xs text-gray-500">
                                    ...
                                    Ci sono già {{ $event->registrations()->count() }} registrazioni per questo evento
                                </p>
                            @endif
                        </div>
                        ...
                        <div x-data="{ requiresRegistration: {{ old('requires_registration', $event->requires_registration) ? 'true' : 'false' }} }">
                            <div class="flex items-center mb-2">
                                <input type="checkbox" id="requires_registration" name="requires_registration" value="1"
                                       class="h-4 w-4 text-rose-600 focus:ring-rose-500 border-gray-300 rounded"
                                       x-model="requiresRegistration" {{ old('requires_registration', $event->requires_registration) ? 'checked' : '' }}>
                ...
                <div class="mt-6">
                    ...
                    <div x-data="{ requirements: {{ json_encode(old('requirements', $event->requirements ?? [])) }} }">
                ...
                <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="flex items-center">
                        <input type="checkbox" id="is_public" name="is_public" value="1"
                               class="h-4 w-4 text-rose-600 focus:ring-rose-500 border-gray-300 rounded" {{ old('is_public', $event->is_public) ? 'checked' : '' }}>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="active" name="active" value="1"
                               class="h-4 w-4 text-rose-600 focus:ring-rose-500 border-gray-300 rounded" {{ old('active', $event->active) ? 'checked' : '' }}>
                        ...
                        <div class="flex">
                            ...
                            <div class="ml-3">
                                <p class="text-sm text-yellow-700">
                                    <strong>Attenzione:</strong> Questo evento ha già {{ $event->registrations()->count() }} registrazioni.
                                    Modifiche a date, prezzo o requisiti potrebbero influire sui partecipanti registrati.
                                </p>
                            </div>
                        </div>
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    startDateInput.addEventListener('change', function() {
        if (this.value) {
            ...
            // Only auto-update if end date is empty or before start date
    ...
    // Auto-update registration deadline when start date changes (only if deadline is empty or after start date)
    const registrationDeadlineInput = document.getElementById('registration_deadline');

    startDateInput.addEventListener('change', function() {
        if (this.value && document.getElementById('requires_registration').checked) {
            const startDate = new Date(this.value);
            const currentDeadline = registrationDeadlineInput.value ? new Date(registrationDeadlineInput.value) : null;

            // Only auto-update if deadline is empty or after start date
    ...
    // Handle max participants change warning
    const maxParticipantsInput = document.getElementById('max_participants');
    const currentRegistrations = {{ $event->registrations()->count() }};

    maxParticipantsInput.addEventListener('change', function() {
        const newMax = parseInt(this.value);
        if (newMax && currentRegistrations > 0 && newMax < currentRegistrations) {
            alert(`⚠️ Attenzione: Il nuovo limite (${newMax}) è inferiore al numero di registrazioni esistenti (${currentRegistrations}). Alcuni partecipanti potrebbero dover essere spostati in lista d'attesa.`);
        }
    });

    // Confirm form submission for events with registrations
    @if($event->registrations()->count() > 0)
    document.getElementById('editEventForm').addEventListener('submit', function(e) {
        const confirmed = confirm('Questo evento ha già delle registrazioni. Sei sicuro di voler continuare con le modifiche?');
        if (!confirmed) {
            e.preventDefault();
        }
    });
