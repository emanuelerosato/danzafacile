#!/bin/bash
################################################################################
# Backup Automatico con Notifica Email
# Esegue backup database e files, carica su Google Drive e invia report via email
################################################################################
...
ADMIN_EMAIL="admin@danzafacile.it"  # Email dove ricevere i report
...
EMAIL_SUBJECT="[Danza Facile] Backup Report - $DATE"
EMAIL_BODY=""
...
<body>
    <div class='header'>
        <h1>$status_icon Danza Facile - Backup Report</h1>
...
# Main script
{
    ...
    add_to_report "💾 BACKUP AUTOMATICO - $DATE"
    add_to_report "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
    ...
    if tar -czf "$BACKUP_DIR/files_${DATE}.tar.gz" -C "$APP_DIR/storage/app" public 2>/dev/null; then
        ...
        print_success "Files: $FILES_SIZE"
        add_to_report "  ✅ Files backup creato: $FILES_SIZE"
    ...
    # 4. Pulizia backup vecchi (mantieni ultimi 7 giorni)
    ...
    if [ "$BACKUP_STATUS" = "SUCCESS" ]; then
        ...
        add_to_report "✅ BACKUP COMPLETATO CON SUCCESSO"
    else
